...
import edu.upb.lp.core.adapter.GameAdapter;
 ...
 *
 */
public class AndroidGameActivity3 extends Activity implements AndroidLibrary,
		OnClickListener {
	...
	public void reproduceSound(String s) {
		...
		for (int i = 0; i < mp.length; ++i) {
			if (!mp[i].isPlaying()) {
				mp[i] = MediaPlayer.create(AndroidGameActivity3.this, uri);
				mp[i].start();
				break;
			}
		}
